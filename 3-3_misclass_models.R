# JAGS Models for aim2_misclass.R
...
model {
  for(i in 1:n){
    # outcome model, log odds of HBV immunity given predictors
    vac[i] ~ dbern(p_vac[i])
    logit(p_vac[i])<- b0+b[1]*reth_hisp[i]+
      b[2]*reth_black[i]+b[3]*reth_asian[i]+b[4]*reth_other[i]+b[5]*age1[i]+
      b[6]*age2[i]+b[7]*fborn[i]+b[8]*incpov1[i]+b[9]*incpov2[i]+b[10]*srvac1[i]
    
    # exposure model, self-reported vaccination given predictors
